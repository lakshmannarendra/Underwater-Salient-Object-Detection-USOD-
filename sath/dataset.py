...
def load_list(dataset_list, data_root):
    # Load images, depths, labels, and contours from the dataset
    images = []
    depths = []
    labels = []
    contours = []
...
def load_test_list(test_path, data_root):
    # Load images and depths for the test dataset
    images = []
    depths = []
    ...
    for depth in depth_files:
        # Append corresponding RGB and depth image paths
        images.append(depth_root.replace('/depth/', '/RGB/') + depth[:-4] + '.png')
        depths.append(depth_root + depth)
...
class ImageData(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        if self.mode == 'train':
            # Load label and contour images for training
            ...
            # For test mode, just apply transformations
